...
******************************************************/
void shortestPath(Graph g, Vertex start, Vertex destination){
	if(start > g->n || destination > g->n){ //checks to see if vertexs are in graph
		printf("A vertex is not in the graph\n");
		return;
		}		

	if(start == destination){ //checks to see if the start and destination vertex are the same, if so, no need to move
		printf("%d->%d\n", start, destination);
		return;
	}
	...
	int i;
	for(i=0; i<g->n; i++){ //sets all values to -1
		visted[i]=-1;
		parent[i] = -1;
	}
	...
	while(removeQ(q, &current)){ //we remove it than look at all the adjacent verticies and que them up
		for(e=firstAdjacent(g, current); e.toVertex!=-1; e=nextAdjacent(g, e)){
			if(visted[e.toVertex]!=1){ //if the current new vertex has not been visted before
				visted[e.toVertex] = 1; //mark as visited
				parent[e.toVertex] = current; //set its parent 
				insertQ(q, e.toVertex); //insert it into the que
				if(e.toVertex == destination){ //the the current vertex is the destination
